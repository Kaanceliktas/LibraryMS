...
public class Category_Author_Publisher_Model {
	public int insert_category(String category_name,String profile,String added_on){
		...
		int flag=0;
	try { 		
		String query="select id_no from category where category_name=?";
	...
	try { 
		...
		String query="select id_no from author where author_name=?";
	...
	try { 
		...
		String query="select id_no from publisher where publisher_name=?";		
		ps=connection.prepareStatement(query);
