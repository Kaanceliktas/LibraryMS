...
public class Issue_Table_Model {
	...
	try { 
					...
					ps.setString(11, obj_Issue_Use_Bean.getIssue_id_no());
	...
	try { 
		...
		if(rs.next()){
			...
			obj_Issue_Use_Bean.setAdmission_no(rs.getString("admission_no"));
							...
							"issue_date," +
							"return_date," +
							"status,issue_id_no,description) values(?,?,?,?,?,?,?,?,?,?,?)";
					...
					ps.setString(5, obj_Issue_Use_Bean.getBook_id_no());
	...
	try { 
		...
		String query="select book_id_no,book_title,book_status from serial_number where bar_code=?";
		ps=connection.prepareStatement(query);
		ps.setString(1, obj_Issue_Use_Bean.getBarcode());
		System.out.println(ps);
		rs=ps.executeQuery();
		...
		if(rs.next()){
			...
			obj_Issue_Use_Bean.setBook_id_no(rs.getString("book_id_no"));
	...
	try { 
					...
					String query="delete from issue_book where issue_id_no=?";
					ps=connection.prepareStatement(query);
					ps.setString(1, obj_Issue_Use_Bean.getIssue_id_no());
					System.out.println(ps);
					flag=ps.executeUpdate();
	...
	return flag;
					...
					try { 
						...
						while(rs.next()){
							...
							obj_Issue_Use_Bean.setIssue_id_no(rs.getString("issue_id_no"));
					...
					public Issue_Use_Bean get_complete_details_of_issue(String issue_id_no){
						Connection connection=null;
						DBConnection_LMS_Portal obj_DBConnection_SMS_Portal=new DBConnection_LMS_Portal();
						connection=obj_DBConnection_SMS_Portal.getConnection();
						PreparedStatement ps=null;
					...
					try { 
						...
						String query="select * from issue_book where issue_id_no=? LIMIT 500";
						ps=connection.prepareStatement(query);
						ps.setString(1,issue_id_no);
						System.out.println(ps);
						rs=ps.executeQuery();
